...
const mockSolTokens: TokenBalance[] = [
  {
    symbol: "SOL",
    name: "Solana",
    balance: "12.5",
    formattedBalance: "12.5 SOL",
    usdValue: "$1,250.00",
    icon: "◎",
    color: "bg-purple-500",
  },
...
export function SolanaTokenBalances() {
  ...
  const fetchBalances = async () => {
    if (!activeAdapter || activeAdapter.id !== "solana" || !account) {
      return
    }
    ...
    try {
      ...
      setTimeout(() => {
        setTokenBalances(mockSolTokens)
        setIsLoading(false)
      }, 1000)
    } catch (err) {
  ...
  useEffect(() => {
    if (activeAdapter?.id === "solana" && account) {
      fetchBalances()
    } else {
      setTokenBalances([])
    }
  ...
  if (!activeAdapter || activeAdapter.id !== "solana" || !account) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-center py-6 text-muted-foreground">Connect a Solana wallet to view your tokens</p>
        </CardContent>
      </Card>
    )
  }
