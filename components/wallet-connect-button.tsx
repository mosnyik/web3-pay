...
export function WalletConnectButton({
  ...
  const { status, disconnect, authenticate, isAuthenticated } =
    useWalletConnection();
  ...
  const isConnected =
    mounted && (status === "connected" || status === "authenticated");

  // connect using rainbowkit if we're on an EVM network and the connected adapter is Ethereum
  if (mounted && isEvmConnected && activeAdapter?.id === "ethereum") {
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-sm font-semibold">
                    {activeAdapter?.name || "Wallet"}
                  </span>
                </div>
                <span className="text-xs text-muted-foreground font-mono font-normal pl-3">
